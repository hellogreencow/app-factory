...
export default function CalendarScreen() {
  ...
  const monthName = 'The Month So Far';
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TouchableOpacity style={styles.addBtn} onPress={() => nav.navigate('DayEntry', { date: now.toISOString().slice(0, 10) })} testID="add-entry" accessibilityLabel="Add Entry">
        <Text style={styles.addBtnText}>Pause to remember today</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  statNum: { color: '#5c5e9b', fontSize: 24, fontWeight: '700' },
  statLabel: { color: '#1a1a2e', fontSize: 12, marginTop: 2 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 24 },
  dayCell: { width: 42, height: 42, borderRadius: 8, backgroundColor: '#f8f9fa', alignItems: 'center', justifyContent: 'center' },
  dayCellDone: { backgroundColor: '#e2e8f0' },
  dayCellSelected: { borderWidth: 2, borderColor: '#6366f1' },
  dayText: { color: '#1a1a2e', fontSize: 14 },
  dayToday: { color: '#6366f1', fontWeight: '700' },
  dot: { width: 4, height: 4, borderRadius: 2, backgroundColor: '#6366f1', position: 'absolute', bottom: 4 },
  addBtn: { backgroundColor: '#6366f1', padding: 16, borderRadius: 12, alignItems: 'center', marginBottom: 40 },
